...
function getSpellingSuggestions(name, possibleSuggestions) {
    ...
    for (const candidate of possibleSuggestions) {
        if (!(Math.abs(candidate.length - nameLowerCase.length) <=
        ...
        const distance = js_levenshtein_1.default(nameLowerCase, candidateNameLowerCase);
...
function quickFix(textDocument, params) {
    const lines = util_1.convertDocumentTextToTrimmedLineArray(textDocument);
    ...
    for (const diag of diagnostics) {
        if (diag.severity === vscode_languageserver_1.DiagnosticSeverity.Error &&
            ...
            const spellingSuggestion = getSpellingSuggestions(diagText, completions_1.getAllRelationNames(lines));
        ...
        if (diag.severity === vscode_languageserver_1.DiagnosticSeverity.Warning &&
